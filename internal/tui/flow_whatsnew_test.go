...
func TestFlow_WhatsNew_MouseScroll(t *testing.T) {
	longContent := testWhatsNewContent + "\n\n" +
		"### Changed\n- Line 1\n- Line 2\n- Line 3\n- Line 4\n- Line 5\n" +
		"- Line 6\n- Line 7\n- Line 8\n- Line 9\n- Line 10\n" +
		"- Line 11\n- Line 12\n- Line 13\n- Line 14\n- Line 15\n" +
	...
	// Mouse wheel down.
	f.Send(tea.MouseMsg{Button: tea.MouseButtonWheelDown, Action: tea.MouseActionPress})
	afterDown := f.Model().whatsNewViewport.YOffset
	if afterDown <= initialOffset {
		t.Errorf("YOffset after wheel down: %d, expected > %d", afterDown, initialOffset)
	}

	// Mouse wheel up.
	f.Send(tea.MouseMsg{Button: tea.MouseButtonWheelUp, Action: tea.MouseActionPress})
	afterUp := f.Model().whatsNewViewport.YOffset
	if afterUp >= afterDown {
		t.Errorf("YOffset after wheel up: %d, expected < %d", afterUp, afterDown)
	}
}
