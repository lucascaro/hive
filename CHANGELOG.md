...
- **`Enter` is now the default attach key** (was `a`). Existing configs with `attach: "a"` are auto-migrated to `"enter"` on schema v6 upgrade; customized values are preserved (#112).
