...
Everything the picker shows outside the small first-party set. Mostly ports of the palettes people arrive with from other editors, added in spec 305. They live in the same file under a `Community` heading and follow every structural rule above — every token re-valued, all sixteen ANSI slots, no per-preset type/space/motion scales.

Two of them — `alucard` and `hex` — are **not ports**. They are original presets that sit in this bucket because it is where the picker puts anything that is not first-party, and the distinction matters for exactly one thing: the contrast exemption below is justified by *preserving someone else's published values*, and there are no published values here to preserve. Both are therefore checked like a first-party preset and pass all sixteen pairs, and neither carries `--contrast-exempt`. The count in the next paragraph is the twelve ports only.

What they do **not** do is meet the contrast bar. They are ported at their published values, so hues, ANSI 16 and chrome greys are upstream's, not re-derived — with one exception: both Solarized presets take `--on-accent: #00161c` (a darkened `base03`) instead of upstream's ink, because the selected sidebar row is an `--accent` fill read in `--on-accent` and upstream's measures 4.08:1 (dark) and 3.41:1 (light) on the Solarized blue; `#00161c` is 5.04:1 (spec 455). The size of that, rather than an impression of it: with the exemption disabled the twelve fail **84** gated pairs between them — Solarized Light 25, Catppuccin Latte 21, Solarized Dark 12, Nord 7, the rest four or fewer. Eleven of the twelve fail `--state-error` on `--sel` — an upstream syntax red on a selection fill — with Nord's `#bf616a` the worst of those at 1.80:1. The worst pair of all is Catppuccin Latte's `--ansi-15` `#bcc0cc` on its own near-white `--term-bg`, at 1.61:1. Reproduce it by flipping the `--contrast-exempt` test in `ui-contrast.mjs` to `false` and re-running the gate. Each of the twelve ported blocks therefore declares `--contrast-exempt: 1` and `ui-contrast.mjs` reports it as skipped rather than pretending it passed. This is a deliberate trade — a user who picks "Dracula" wants Dracula, and these are opt-in — but it is a real accessibility cost, and the six presets above (the only ones a user can land on *without* opting in) stay strictly gated. `ui-contrast.mjs` refuses the marker on those six by name.

| Name | Basis | Notes |
|---|---|---|
| `dracula` | Dracula | `#21222c`/`#282a36` grounds, purple accent (pink is spoken for by `--state-error`). Dracula's own published ANSI |
| `alucard` | Original — **not a port** | Castlevania's dhampir, not the Dracula org's light theme of the same name. Sampled off Kojima's Symphony of the Night promo art, which is why it is not red: red-hued pixels above 25% saturation are **0.06%** of that painting. Warm-black grounds `#0a0908`/`#13110f` (half the art sits under 12% luminance), cream type `#ece4d8` (the hair), gold accent `#d9a862` (the filigree, mean `#b88f5e`) with `--on-accent #14120f` at 8.7:1, and the moon/candle steel blue `#8fa8ae` as `--state-info`. Red is kept for `--state-error` alone, where it has to mean something. Accent and `--state-attention` sit close in hue on purpose — `hive-dark` ships the same pair. **Gated, not exempt** |
...
| `hex` | Original — **not a port** | The dark-purple one. Ink-violet grounds `#0c0813`/`#150f20`, amethyst accent `#a978ff` on a dark `--on-accent #16102a` (6.0:1), saturation kept in the chrome so the type stays near-neutral. The state family stays in its usual four hues — a purple preset still has to say "running", "needs you" and "error" in something that is not the accent. **Gated, not exempt** |
...
Fixtures, one per branch, each must fail: `scripts/testdata/ui-contrast/exempt-default.css` (the `NEVER_EXEMPT` refusal), `exempt-incomplete.css` (completeness), `exempt-light-no-ansi.css` (the ANSI half of completeness). `good.css` carries the positive case — an exempt preset whose every pair is far below threshold and which must still pass. They are separate files on purpose: folded into `bad.css` they would prove nothing, since it already exits 1 for dozens of other reasons. Each one is verified by **mutating its branch out and checking the fixture then exits 0** — "it fails" is not the same as "it fails for the reason claimed", and the first cut of `exempt-default.css` declared only the marker, so it tripped completeness and would have stayed green with the refusal deleted.
...
2. Add ANSI 16 (`--ansi-0 … --ansi-15`). A preset on a light ground **must** re-value all sixteen: the defaults are xterm's Tango palette, and seven of those fail WCAG AA on white — `brightWhite` lands at 1.16:1, i.e. invisible. A first-party light preset is checked on the ratios by `test/e2e/theme.spec.ts`, which computes the contrast rather than pinning hexes; that loop is first-party-only, so an exempt light preset is instead required by `ui-contrast.mjs` to *declare* all sixteen. Neither check can live in the e2e alone: it reads `getComputedStyle`, which resolves through the cascade, so an omitted slot comes back as the inherited value and looks present.
...
4. Screenshot baselines — **first-party presets only**. `test/e2e/theme.spec.ts`'s `preset baselines` block loops over `PRESETS` minus the `Community` group, so a first-party preset gets sidebar, dialog, worktrees and launcher tests by existing: run `HIVE_SNAPSHOT=1 npx playwright test test/e2e/theme.spec.ts --update-snapshots` on macOS and read the four new PNGs — they are darwin-local and default-skipped, which is why they must be eyeballed rather than trusted.

   Community presets are excluded deliberately. Twelve of them would add ~48 darwin-local PNGs that CI never runs, and because the block is skipped without `HIVE_SNAPSHOT` the absence is invisible — the first person to run it locally would have Playwright *write* all 48 and pin whatever rendered that day. The failure these baselines catch (a token falling through to `hive-dark`) is caught structurally for an exempt preset instead, by the completeness rule above. If you want the pixels anyway, drop the group filter in that loop and eyeball every shot before committing.
