...
Selection owns the row's *static* background. Attention never paints a static background — but it may pulse one, which is a channel selection does not use. On an unselected row the pulse is `--state-attention` at 12%. On the SELECTED row an attention tint would vanish into the fill wherever the two hues meet, so the pulse there is a 2px inset `--on-accent` border, never a tint (a tint over the fill would lower the ink's contrast: 18% took Solarized to 3.79:1), and the state icon keeps its diamond and its attention colour on a `--surface` disc. Under `prefers-reduced-motion` the pulses become static (a 9% attention tint; the selected row's border stays on); an unselected attention row's tint must stay quieter than the selection, which the theme tests assert, and the selected row's cue is asserted in `sidebar-legibility.spec.ts`.
