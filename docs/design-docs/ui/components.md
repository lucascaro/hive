...
- Line 1: name, `--text-md`, `--fg` when selected else `--fg-muted`; attention → `--state-attention` + weight 500; exited/error → `--fg-subtle` + `text-decoration: line-through` (see patterns.md for why not hidden). The name is rendered verbatim. Generated names no longer carry the agent id at all (`internal/agent/names.go`, `internal/registry/create.go`), which is where that duplication was fixed — never at display time: nothing on the wire distinguishes a generated name from one the user typed, so trimming a trailing agent id would silently rewrite a session deliberately named "weekly claude".
...
- The only CONTAINER in the sidebar tree: a full-bleed band, `border-top` + `border-bottom` 1px `--border` on a `--surface-raised` ground, square. Containment means exactly one thing — these sessions share a working directory. It was a rounded, inset card until #392: next to the flat project label the rounding read as leftover card vocabulary, and the 8px side margin cost a member row 18px of window title against an ungrouped row. Full bleed means member rows start exactly where ungrouped rows start, and the panel's colour bar lands on the same x as a row's own.
...
- A flat LABEL, not a card: no border, no fill, no side margins. The worktree group band is the only container in the tree, and it is square and full-bleed too — a rule plus a header band is the sidebar's whole grouping vocabulary.
