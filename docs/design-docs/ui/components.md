...
- Attention: an `::after` overlay pulsing `--state-attention` at 12% on `--motion-pulse`, flat 9% under `prefers-reduced-motion`. An overlay rather than the row's own background, because an animation's value beats a normal declaration and would override selection's fill (patterns.md › Selection vs attention). On a selected row the overlay paints no tint — any tint over the fill lowers the ink's contrast — but a 2px inset `--on-accent` border on the same pulse (steady under reduced motion).
