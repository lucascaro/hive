...
- Shared worktree: when two or more sessions in the project are backed by the same worktree path, every one of those rows gets `data-wt-shared` and a 3px `--session-color` bar at the RIGHT edge (`::after`), and the branch icon gains a `--fg-subtle --font-mono --text-xs` count of the group. Sessions that adopt a worktree inherit the colour of the session they joined (`internal/registry/create.go`), so the bar is the same on every member. The count exists because the colour must never be the only channel; the branch button's `aria-label` says the same thing in words. Grouping, the drag, and the keyboard reorder are pure functions in `src/lib/worktree-groups.ts`; a worktree with no branch (detached) still gets the glyph so the cue is never colour-only. See patterns.md › One order for the ordering rule and the two reorder gestures.
