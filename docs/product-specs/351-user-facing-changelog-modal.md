...
- Any change to how `.changesets/*.md` are written or rolled up. `scripts/release.sh` is untouched, and changeset schema and rollup are unchanged. (`scripts/regen-generated.py` *is* touched, by one addition: `--release <version>` also stamps `since: "Unreleased"` entries in the feature list, the same way it promotes the changelog's `[Unreleased]` section. That fell out of review — without it a PR cannot add its own feature entry, because a new feature has no released version to name.)
