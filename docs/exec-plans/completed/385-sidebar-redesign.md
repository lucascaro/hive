...
- **2026-09-11** — A group member with no window title falls back to the shared branch-derived name, and gets no per-session cue. Why: the condition is transient (any agent emits an OSC title within seconds) and three other channels already separate the rows — state icon, agent glyph, ⌘-hint. Every stable cue costs more than it buys: a session-id fragment puts an opaque token in the line the redesign exists to make readable, and the agent name only separates different agents, so two claude sessions on one branch would still read alike.
- **2026-09-11** — `daemon-contract-override` rather than a DaemonContract bump for the naming change. Why: only the TEXT of a generated name changed — no wire shape, no new field, no behaviour a client can observe — and the check's own message notes that a needless bump costs every user their running sessions.
