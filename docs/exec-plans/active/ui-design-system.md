...
- **2026-08-30** — The three state-icon sites are patched in place via `updateStateIcon()` from the existing repaint paths (`updateSidebarSelection`, the tile's `refreshStateIcon`, `renderMinimizedTray`). Why: a bell only toggles a class, so without this the shape and its `<title>` keep saying "Running" while the session waits.
