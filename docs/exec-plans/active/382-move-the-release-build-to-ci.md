...
- **2026-09-09** — verdict: NEEDS_FOLLOWUP; phase: —; checks: 17 passed / 0 failed / 2 followups; followups: none filed (both are closable on this branch); one-line: every criterion verifiable today passes and both rehearsals succeeded, but the `--prerelease` fix has never run live — it landed after both rc runs.
  - 2026-09-09 dimensions:
    - acceptance — NEEDS_FOLLOWUP — 8 of 10 criteria PASS with live evidence (selftest 35/35, shellcheck + actionlint clean, `preflight ok: team=2ZY25TNMX6 signing=off`, both rc runs `success`, executed step order shows build before certificate import, `/releases/latest` = `v2.7.0`, environment carries a required reviewer and a `v*`-tag-only policy). Two held: (a) the `--prerelease` behaviour is covered only by the mocked selftest — `f55f9437` landed 15:36Z, after the rc1 (04:06Z) and rc2 (04:44Z) runs, and rc2's own log shows `Creating GitHub release v2.7.1-rc2 (draft)...` with no prerelease suffix, so the fixed code has never executed for real; (b) `spctl`/`stapler`/checksum verification on rc2 passed when the maintainer ran it in-session with recorded output, but the rc releases were deleted afterwards so the validator could not independently re-run it.
