...
- **Laya** (`internal/laya/`) — the optional client for a user-run Laya decision model, and the labelled screen corpus it is scored against (spec 458, [laya-state-classifier.md](docs/design-docs/laya-state-classifier.md)). Hive's only outbound HTTP besides the GUI's update checks. Imported by `internal/daemon/`, which classifies sessions, by `cmd/hivegui/` for the Settings screen's connection test, and by the offline `scripts/laya-scrub` for its secret scrubber. It is never imported by the registry: the registry decides *when* to classify a session and takes the call as a plain `registry.Classifier` func, so it never learns about HTTP, settings or the API key, and the call is always made with the registry mutex released. Every call goes through `laya.NewClient`, which never follows a redirect.
