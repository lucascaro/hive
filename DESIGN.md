...
- **Agent state** (`internal/agentstate/`) — the state machine behind `SessionInfo.state`: working / idle / waiting_input / waiting_permission / exited / error, plus which tier reported it. Pure domain code with no clock and no locks — every method takes `now` from its caller — so the registry can drive it from PTY output, agent hooks and agent extensions without those three agreeing on anything. Its single owner is `registry.Entry`, under the registry mutex. It also holds each session's **activity** — a bounded in-memory ring of recent tool calls plus the latest plan snapshot ([agent-activity.md](docs/design-docs/agent-activity.md)) — for the same lifecycle reason: a fresh `Machine` on create/restart/revive discards it, and deleting the entry frees it, so there is no second place to clear. Activity is in-memory only and dies with the daemon, like the PTY.
- **Laya** (`internal/laya/`) — the optional client for a user-run Laya decision model, and the labelled screen corpus it is scored against (spec 458, [laya-state-classifier.md](docs/design-docs/laya-state-classifier.md)). Imported by `internal/daemon/`, which classifies sessions, by `cmd/hivegui/` for the Settings screen's connection test, and by the offline `scripts/laya-scrub` for its secret scrubber. It is never imported by the registry: the registry decides *when* to classify a session and takes the call as a plain `registry.Classifier` func, so it never learns about HTTP, settings or the API key, and the call is always made with the registry mutex released. Every call goes through `laya.NewClient`, which never follows a redirect.
- **Plugins** (`internal/plugin/`) — headless plugins: user-installed programs `hived` supervises as child processes. Owns the manifest (`hive-plugin.json`), installs (a local directory copy, or a shallow `git clone` pinned to its commit), and a `Manager` that runs one supervisor per *enabled* plugin — restart with backoff, a crash-loop cutoff, a process-tree kill. A plugin is an ordinary wire client on a socket of its own: the daemon opens a fresh `<sock>.plugin-<hex>` listener per run and tags every connection on it, so a plugin gets exactly the verbs a GUI gets, is never counted as a client that can answer a dialog, and spends from a per-plugin rate budget. Author guide: [docs/plugins.md](docs/plugins.md).
...
- **Child processes** — every non-PTY child (git, gh, `claude --version`, `tasklist`, the staged bundle's `hived --version`) is created through `internal/proc/`. It exists because `hivegui.exe` is a GUI binary and `hived.exe` is started detached, so neither owns a console: on Windows a console child spawned from either one is handed a brand new console *window*. PTY children are `internal/session/`'s and go through a ConPTY instead.
...
- **Cross-platform parity is verified per release.** `notify`, `worktree`, `os_terminal`, and PTY paths all have platform splits — every release exercises macOS, Linux, and Windows builds (`scripts/release-artifacts.sh`, run by `.github/workflows/release.yml`). No `runtime.GOOS == "darwin"` shortcuts in domain code; gate at the package boundary.
- **Child processes go through `internal/proc`, never `os/exec` directly.** `proc.Command` / `proc.CommandContext` mark the child `CREATE_NO_WINDOW` on Windows; a direct `exec.Command` compiles and runs fine, and puts a console popup on screen for every Windows user on every call. Enforced by `TestNoDirectExecOnWindows` (`internal/proc/edict_test.go`), which scans every Windows-buildable file and carries the allowlist: the two detached spawns in `cmd/hivegui/` (`DETACHED_PROCESS` already leaves the child consoleless, and Windows ignores `CREATE_NO_WINDOW` when it is set alongside it) and `os_terminal.go` (where a visible window is the point).
