...
- **Laya** (`internal/laya/`) — the optional client for a user-run Laya decision model, and the labelled screen corpus it is scored against (spec 458, [laya-state-classifier.md](docs/design-docs/laya-state-classifier.md)). The daemon's only outbound HTTP. Imported by `internal/daemon/` alone: the registry decides *when* to classify a session and takes the call as a plain `registry.Classifier` func, so it never learns about HTTP, settings or the API key, and the call is always made with the registry mutex released.
