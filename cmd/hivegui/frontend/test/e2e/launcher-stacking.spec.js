...
test('agent launcher paints above the sidebar resize bar (even when hovered)', async ({
  ...
  await page.waitForSelector('#launcher:not(.hidden) .launcher-item', {
    timeout: 5000,
  });
  ...
  const pts = await page.evaluate(() => {
    ...
    const overlapY =
      (Math.max(L.top, R.top) + Math.min(L.bottom, R.bottom)) / 2;
    const overlaps =
      Math.min(L.right, R.right) > Math.max(L.left, R.left) &&
  ...
  expect(
    pts.overlaps,
    'launcher must overlap the resize strip for this test to mean anything',
  ).toBe(true);
  ...
  const { overOverlap, overStrip } = await page.evaluate(
    async ({ b64, pts }) => {
      const img = new Image();
      ...
      return {
        ...
        overStrip: px(pts.stripX, pts.stripBelowY),
      };
    },
  ...
  expect(
    isAmber(overStrip),
    `hover should make the bare strip amber, got ${overStrip}`,
  ).toBe(true);
  // The real assertion: the strip's amber must NOT bleed over the menu.
  expect(
    isAmber(overOverlap),
    `resize strip painted over the launcher, got ${overOverlap}`,
  ).toBe(false);
});
