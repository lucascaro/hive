...
test('launcher shows a loading row before agents resolve', async ({ page }) => {
  await boot(page);
  // The mock resolves ListAgents on a microtask, so the loading row is
  // gone by the time we can assert — verify the structure instead by
  // checking that the launcher opens synchronously on ⌘T.
