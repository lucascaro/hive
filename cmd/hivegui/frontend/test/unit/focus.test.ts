...
describe('decideFocusAction', () => {
  ...
  it('clears when a modal is open', () => {
    expect(decideFocusAction(snap({ modalOpen: true })).kind).toBe(
      ACTION_CLEAR,
    );
  });
  ...
  it('preserves when a real INPUT owns the keyboard', () => {
    const a = decideFocusAction(
      snap({ activeTag: 'INPUT', activeClasses: 'tile-name-input' }),
    );
  ...
  it('preserves when a real TEXTAREA (non-xterm) owns the keyboard', () => {
    const a = decideFocusAction(
      snap({ activeTag: 'TEXTAREA', activeClasses: 'something-else' }),
    );
    expect(a.kind).toBe(ACTION_PRESERVE);
  });
  ...
  it('focuses through when the xterm helper-textarea is already active', () => {
    const a = decideFocusAction(
      snap({ activeTag: 'TEXTAREA', activeClasses: 'xterm-helper-textarea' }),
    );
  ...
  it('accepts an array-shaped knownTermIds', () => {
    expect(decideFocusAction(snap({ knownTermIds: ['s1'] })).kind).toBe(
      ACTION_FOCUS,
    );
    expect(
      decideFocusAction(snap({ id: 'sZ', knownTermIds: ['s1'] })).kind,
    ).toBe(ACTION_CLEAR);
  });
