...
- **Mouse support** — click sidebar items, preview pane, and grid cells; scroll wheel in all panes
- **Live preview** — 500ms refresh with ANSI color passthrough; always shows the most-recent output
...
Press `g` to open a tiled overview of all sessions in the current project. Press `G` to see all sessions across every project. Each tile shows a live preview of the session's most-recent output and displays the **project name**, **session name**, and **agent type**.
...
You can also **left-click** a tile to move the cursor to it, or click the hint bar at the bottom to navigate.
